...
\author{%
  ...
  \textsuperscript{1}\textit{ECE Paris, School of Engineering}\\
  \texttt{\{hamza.khan, alexandre.danoffre\}@edu.ece.fr}%
}
\date{April 2026}
...
\footnotetext{OOS Sharpe and Return columns report W3 (2025) performance only. Multi-window averages (Table~\ref{tab:multiwindow_results}) span three regimes and would yield different rankings; the single-year values are shown here because the threshold sensitivity analysis applies the same Phase~A gate uniformly and the W3 OOS year is the primary test period.}
...
\begin{itemize}
  \item At $\tau=0.65$, binary FPR drops to 0\% ($0/6{,}000$ trials) while ternary FPR is 0.12\% ($7/6{,}000$). The residual ternary false positives come from 10d ternary (5 seeds) and 7d ternary (1 seed), reflecting occasional alignment of noise-corrupted features with the SCS sub-score thresholds.
...
The complete pipeline (approximately 1,900 lines of Python, 42 automated tests, 14,580 model runs, 135,000+ trades) is open-source and fully reproducible.


% ============================================================
\section*{Code Availability}

The complete implementation---including the performance-weighted $S_{\text{model}}$, multi-window OOS pipeline, FDR simulation, and power analysis---is available at \url{https://github.com/HamzaKhan2002/scs-framework}. The repository contains approximately 1{,}900 lines of Python across 22 modules, 42 automated tests, and all experiment result files needed to reproduce every number in this paper.
...
The backtest engine proceeds as follows. Each trading day involves four steps: (1)~check stop-loss/take-profit on open positions using directional return; (2)~close positions that have reached their planned horizon; (3)~rank tickers by model confidence, apply probability thresholds ($P \geq 0.55$), and open new positions subject to position limits; (4)~mark-to-market including unrealized P\&L. Positions are entered at the closing price of the signal date; in practice, execution would occur at or near the subsequent open, introducing an implementation shortfall not modeled here. Transaction costs are fixed basis points on traded notional (5~bps baseline, 15~bps stress) with no market-impact or slippage model. The annualized Sharpe ratio is $\text{SR} = (\bar{r}_d / \sigma_d) \times \sqrt{252}$, computed from the daily equity curve.
